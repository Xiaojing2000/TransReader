...
public:
  ...
  // TransReader patch: in-memory variant of Predict() that takes decoded BGR
  // images directly. Only supported when use_doc_preprocessor_ == false.
  std::vector<std::unique_ptr<BaseCVResult>>
  PredictMats(const std::vector<cv::Mat> &input);
...
public:
  ...
  // TransReader patch: in-memory passthrough for the single-threaded
  // (thread_num_ == 1) configuration used by TransReader.
  std::vector<std::unique_ptr<BaseCVResult>>
  PredictMats(const std::vector<cv::Mat> &input) {
    if (thread_num_ != 1 || !infer_)
      return {};
    return static_cast<_OCRPipeline *>(infer_.get())->PredictMats(input);
  }
