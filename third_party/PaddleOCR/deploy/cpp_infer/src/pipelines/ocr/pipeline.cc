...
// TransReader patch: in-memory variant of Predict() above. Accepts decoded
// BGR cv::Mat images directly so callers (trans_ocr.cpp) can skip the BMP
// encode + disk write + imread round-trip that the path-based Predict()
// performs via ImageBatchSampler. Only the use_doc_preprocessor_ == false
// configuration is supported (TransReader never enables it).
std::vector<std::unique_ptr<BaseCVResult>>
_OCRPipeline::PredictMats(const std::vector<cv::Mat> &input) {
  if (use_doc_preprocessor_) {
    INFOE("PredictMats does not support the doc preprocessor pipeline");
    return {};
  }
  auto model_settings = GetModelSettings();
  std::vector<std::vector<cv::Mat>> batches = {input};
  std::vector<std::unique_ptr<BaseCVResult>> base_results = {};
  pipeline_result_vec_.clear();
  for (int i = 0; i < batches.size(); i++) {
    // TransReader patch: the origin_image clone collection in Predict() is
    // dead code (cloned but never read downstream), so it is omitted here.
    std::vector<DocPreprocessorPipelineResult>
        doc_preprocessors_pipeline_results = {};
    // The guard above guarantees use_doc_preprocessor_ == false, so the
    // images pass through unchanged (same as the else branch in Predict()).
    DocPreprocessorPipelineResult result;
    for (auto &image : batches[i]) {
      result.output_image = image.clone();
      doc_preprocessors_pipeline_results.push_back(result);
    }
    std::vector<cv::Mat> doc_preprocessor_pipeline_images = {};
