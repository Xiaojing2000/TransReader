...
// TransReader patch: in-memory variant taking decoded BGR images.
std::vector<std::unique_ptr<BaseCVResult>>
PaddleOCR::PredictMats(const std::vector<cv::Mat> &input) {
  return static_cast<OCRPipeline *>(pipeline_infer_.get())->PredictMats(input);
}
