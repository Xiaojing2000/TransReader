...
bool Utility::IsMkldnnAvailable() {
  ...
  // oneDNN supports modern x86-64 CPUs from both Intel and AMD. The upstream
  // Intel-only brand check unnecessarily disables the optimized path on Ryzen.
  if (brandStr.find("Intel") != std::string::npos ||
      brandStr.find("AMD") != std::string::npos) {
    return true;
  }
  return false;
#else
