...
TransReader 当前只支持 Windows x64。两个安装包功能完全相同，本地 AI 模型都在首次使用时按需下载，不会塞进安装包。

| 文件 | 适合谁 | 是否写入系统 |
| --- | --- | --- |
| `TransReader-vX.Y.Z-win-x64-setup.exe` | 大多数用户，推荐 | 安装到当前用户目录，创建开始菜单项，可选桌面快捷方式和标准卸载入口 |
| `TransReader-vX.Y.Z-win-x64-portable.zip` | 便携使用、没有安装权限或希望自行管理目录 | 不注册安装信息，解压后直接运行 |

`X.Y.Z` 是版本号，例如 `TransReader-v0.3.0-win-x64-setup.exe`。
...
- 约 600 MB 可用磁盘空间；安装本地 AI 后还需约 1.3 GB。
...
1. 下载 `TransReader-vX.Y.Z-win-x64-portable.zip`。
2. 将整个压缩包解压到一个可写目录，例如 `D:\Apps\TransReader`。
3. 不要直接在压缩包预览窗口中运行，也不要只复制单个 EXE。
4. 双击 `TransReader.App.exe`。

便携版的程序文件可以随目录移动，但设置、缓存和文献库仍保存在 `%LOCALAPPDATA%\TransReader`，以避免升级时误删资料。

## 第一次使用：三分钟开始阅读

1. 打开 TransReader，点击“打开 PDF”，选择论文、说明书、电子书或扫描文档。
2. 选择翻译方式：
   - **在线 API**：进入“AI 中心”，选择预设或添加任意 OpenAI 兼容端点，填写模型名和 API Key，然后测试连接。
...
API Key 只保存在 Windows 凭据库，不会写进 `settings.json`。在线模式会向你配置的端点发送当前页图像或 OCR 文本、必要的跨页上下文以及你的问题；具体内容取决于模型是否启用多模态。本地模式不会把页面内容发送到外部 API。

如果文档敏感，请使用本地模式，并确认“文献库整理”和“阅读助手”的模型来源也设为本地。

## 更新

- **Setup 版**：退出 TransReader，运行新版本 Setup。安装器会覆盖程序文件，不会删除 `%LOCALAPPDATA%\TransReader` 中的文献库和设置。
- **Portable 版**：退出应用，解压新版本到新的目录，确认运行正常后再删除旧程序目录。不要覆盖仍在运行的目录。

## 卸载

- Setup 版：打开 Windows“设置 → 应用 → 已安装的应用”，搜索 TransReader 并卸载。
- Portable 版：退出应用后删除解压目录。
...
不要从安装目录删除 `models`、`TransOcrNative.Host.exe` 或旁边的 DLL。Portable 用户应重新完整解压；Setup 用户可重新运行安装包修复。
